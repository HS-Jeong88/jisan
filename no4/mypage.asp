	...
	<body>
		...
		<div class="sub mypage">
					...
					<ul class="clearfix">
						...
						<li class="arrow sub_menu">
							...
							<ul>
								...
								<% If NullChk(Session("js_id")) And NullChk(Session("sns_id")) Then %>
								<%If Not IsEmpty(js_id) Then%>
								<li><a href="/w2/member/mypage/member_pwc.asp" class="">회원정보수정/탈퇴</a></li>
								<% Else %>
								<li><a href="/w2/member/mypage/member_info.asp" class="">회원정보수정</a></li>
								<% End If%>
			...
			<div class="inner mt230 pb143">
				...
				<% If NullChk(Session("js_id")) And NullChk(Session("sns_id")) Then %>
				<% Else If Not IsEmpty(js_id) Then%>
				...
				<div class="total_wrap txt_center pt10 pb88 total_wrap2">
					...
					<li>
						...
						<li>
							<p>퍼블릭 예약조회</p>
							<div><a href="../reservation/golfResv/public_reserv_list.asp"
									class="rel fs16 white brown_btn mt40">바로가기</a></div>
						</li>
						<li>
							<p>스코어조회</p>
				...
				<div class="total_wrap txt_center pt10 pb88 my_detail ski_list">
					<ul class="clearfix">
						...
						<li>
							<p>스키&보드<br>강습 예약</p>
							<div><a href="./mypage/lesson01_list.asp" class="fw600 fs60">
									<%=lessonCnt%>
								</a> <span class="fs28">건</span></div>
						</li>
						<!-- <li>
						<p>개인 락카함<br>구매</p>
						...
						<tbody>
							...
							For i=0 To UBound( arrTicket, 2) 
								...
								<tr>
									...
									<td>
										<%=fn_buytypeName(buytype)%>
										<% tmpMemtype=memtype 
											If memtype="6" And buytype <> "personal" Then tmpMemtype = "1"
											End If
											If memtype = "6" And s_usetime <> "1" Then tmpMemtype = "1"
											End If %>
									...
									<td>
										<%=fn_accountName(s_account_method)%> (<%=fn_accountChkName(s_account_check)%>)
									</td>
									<td>
										<%=putdate(reg_ymd,"YYYY.MM.DD")%>
									</td>
								</tr>
							<% Next Else %>
								<tr>
				...
				<div class="total_wrap txt_center pt10 pb88 my_detail ski_list sns_ski_list">
					<ul class="clearfix">
						...
						<li>
							<p>회원정보 수정</p>
							<div><a href="/w2/member/mypage/member_info.asp" class="rel fs16 white brown_btn mt40">자세히 보기</a></div>
						</li>
					</ul>
				</div>
				<!-- //sns 로그인 시 -->
				...
				<div class="pt10 pb69 rel">
					...
					<table class="mypage_table txt_center">
						...
						<tbody>
							<% If isArray(arrQnA) Then For i=0 To UBound( arrQnA, 2) '// 출력
							...
							<tr>
								...
								<td>
									<a href="javascript:void(0);" onclick="web.pageMove(' <%=moveUrl%>','seq=<%=arrQnA(0, i)%>')"><%=arrQnA(2, i)%></a>
								</td>
								<td>
									<%=putDate(arrQnA(6, i),"yyyy-mm-dd")%>
								</td>
								<td>
									<span class="<%=stat_cs%>">
										<%=stat_nm%>
